(function (window) {
    /** 说明
     * 创建一个symbol符号分2步走：
     * step1 确定符号：var iconFn = symbols.cross;
     *       这样也行：var iconFn = symbols["cross"];
     * step2 配置并输出： var icon = iconFn({scale: 1, fillColor:"blue"})
     * 
     */

/*  
    由于百度地图无法正确解析svg中的path元素的d值，
    故此，采用这种混合的配置方式：path 或者 SymbolShapeType
    let shp = {
        ...
        arrow_bwc: BMap_Symbol_SHAPE_BACKWARD_CLOSED_ARROW,
        arrow_fwc: BMap_Symbol_SHAPE_FORWARD_CLOSED_ARROW,
        arrow_bwo: BMap_Symbol_SHAPE_BACKWARD_OPEN_ARROW,
        arrow_fwo: BMap_Symbol_SHAPE_FORWARD_OPEN_ARROW
    };

    // shp.circle/shp.drop/shp.cuteDrop无法渲染出来，百思不得其解！！！
    let shp = {
...
*/    
    ...
    /**
     * 这个函数主要的用途是快速构建BMap.Symbol
     * fn.path, fn.anchor用于配置变化的参数
     * 配置完后直接运行fn()就可以产生一个BMap.Symbol实例
     */
    let makeSym = function () {
        let _path, _anchor = size(16, 16);
    
        let fn = function (opt) {
            if (!_path) {
                throw Error("path必须配齐！");
            }
            ...
            Object.assign(defaultOpt, opt, {anchor: _anchor});
        ...
        // 这里的path可以是d值，也可以是SymbolShapeType
    ...
    let symbols = {
        circle:   makeSym().path(shp.circle).anchor(size(0, 0)),
